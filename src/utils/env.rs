...
/// Get the `LOG_LEVEL` environment variable.
pub fn get_log_level() -> Result<String> {
    let log_level = std::env::var("LOG_LEVEL")?;
    Ok(log_level)
}

/// Set the `RUST_LOG` level environment variable.
pub fn set_log_level(level: impl Into<String>) {
    unsafe {
        std::env::set_var("RUST_LOG", level.into());
    }
}
